...
$sphinx->SetServer( '127.0.0.1', 9301); #localhost', 3312 );
...
if ($query)
{
   my ($id, $author) = geo_query_search($query, '', $DEBUG, 'verbose');
...
#	    print "$tID $text<br/>";
    ...
    {
	my $original = $text;
	my (@chains, %lexicon, $lexchain, $newtext, @coordinates, $keyID);
...
# 	print "$IDline\n";
	...
	foreach $text (sort keys %candidates)
	{
	   if (length($text) > 1)
	   {
	     my ($id, $author, $rank, $typehash, $typeamount) = geo_query_search($text, '', $DEBUG);
		...
		if (!$max || ($thistypes{$type} > $max))
		{
	     ...
	     my %coordinates = get_coordinates($dbh, $id);
#latitude          : latitude in decimal degrees (wgs84)
#longitude  
	     print "$text;;$author;;$id;;$MARC21full{$typeselected};;$coordinates{$id}{latitude};;$coordinates{$id}{longitude} <br>";
	   }
	} 
	...
	foreach $text (@chains)
	{
	    ...
	    unless ($author)
	    {
		($id, $author, $rank, $typehash, $typeamount) = geo_query_search('', $text, $DEBUG);
		...
		$text=~s/^\s+|\s+$//g;
		print "$text;;$text;;$id;;$selectedtype $br\n";
...
sub geo_query_search
{
	...
	$id = $results->{matches}->[$x]->{geonameid};
	$type = $results->{matches}->[$x]->{type};
	%info = %{$results->{matches}->[$x]};
	$alternatives = $results->{matches}->[$x]->{alternatenames};

#	$name = $results->{matches}->[$x]->{name};
#	print "$name <br>\n";
	$i++;
	$rating{$doc} = $i;
	#unless ($authID)
  	#my @allitems = ($query);
 	my @extra = split(/\,/, $alternatives);
	my @allitems;
 	push(@allitems, @extra);

# 	print "@allitems $alternatives\n";

	foreach $entity (@allitems)
	{
	   my $score = $mod->getSimilarityStrings($entity, $query);
	   $rank{$entity} = $score;
#	   print "$doc $entity $type $score <br>\n";
	   $IDs{$entity} = $id;
	   $types{$type}++ if ($type);
	   $amount++ if ($type);
...
#longitude         : longitude in decimal degrees (wgs84)
        ...
        $sqlwords=~s/\,\s+$//g;
        $sqlquery = "select latitude, longitude from geonames where geonameid in ($id)";
        my $sth = $dbh->prepare("$sqlquery");
        $sth->execute();

        while (my ($latitude, $longitude) = $sth->fetchrow_array())
        {
	    $geo{$id}{latitude} = $latitude;
	    $geo{$id}{longitude} = $longitude;
        }
    };
