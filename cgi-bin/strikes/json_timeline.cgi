...
sub getrecords
{
	...
	if ($img)
	{
	    ...
	    my $eventID = $id;
	    ($eventhash, $stathash) = get_events($dbh, $year, $month, $day, $eventID);
	    %events = %{$eventhash} if ($eventhash);
	   
	    foreach $key (sort keys %events)
	    {
	 	$key=~s/\r|\n//g;
		($id, $date, $year, $profession, $cities, $company) = split(/\;\;/, $key);
		$text.="<b>Event Description</b><br /> http://www2.iisg.nl/databases/stakingen/details.asp?id=$id <br />";
		$text.="Profession: $profession <br /> Location: $cities <br /> Companies: $company <br />";
	    }
...
$json.="
            {
                ...
                \"text\":\"$text <br /><b>Published: $newsTitle</b><br /> <a href='$img' target='_blank'>$img</a>\",
                \"asset\":
                {
                    \"media\":\"$image\",
                    \"credit\":\"$id\",
